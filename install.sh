...
check_token() {
  ...
  if [ "$USER_TOKEN" = "babatung" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Tuxz${NC}"
    echo -e "${YELLOW}TELEGRAM : @reyzi${NC}"
    echo -e "${YELLOW}WHATSAPP : +6282114081878${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE${NC}"
    echo -e "${YELLOW}© Tuxz${NC}"
    exit 1
  fi
  clear
}
